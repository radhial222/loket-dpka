...
Do Case
	...
	Case lcPlat = 'KUNING'
		*mobil beban = njkb * bobot * 0.008 dan mobil penumpang = njkb * bobot * 0.006
		nk = Iif(clJenisKend= 'F' Or clJenisKend ='G',0.5 * 0.0075 , 0.3 * 0.0075)
...
CASE lcPlat = 'KUNING'
	...
	If (clJenisKend= 'F' Or clJenisKend ='G')
		njkb_hitung=Transform(lnNilai,'999,999,999')+' x '+Transform(bobot)+' x '+Transform(persen*100,"999.99")+'% x 50% = '+Transform(lnPkb,'999,999,999')
	Else
		njkb_hitung=Transform(lnNilai,'999,999,999')+' x '+Transform(bobot)+' x '+Transform(persen*100,"999.99")+'% x 30% = '+Transform(lnPkb,'999,999,999')
	ENDIF
...
*!*			disini
*!*	messagebox(transform(ldbatas1))
*!*	messagebox(transform(ldbatas3))
...
*!*		IF (dltgl_notice < CTOD(batas))
*!*			hari=TRANSFORM(DAY(dltgl_notice))
*!*			bulan=TRANSFORM(MONTH(dltgl_notice))
*!*						
*!*			*mulaiDenda=CTOD(batas)
*!*			mulaiDenda=CTOD(hari+'/'+bulan+'/'+'2013')
*!*		ELSE
*!*			mulaiDenda=dltgl_notice	
*!*			*MESSAGEBOX(TRANSFORM(mulaiDenda))
*!*		ENDIF 
*!*	*!*		messagebox('dispensasi')
*!*	*!*		messagebox(dispensasi)
*!*		&&queri disini
...
*!*		SQLExec(gnconnhandle,'SELECT TIMESTAMPDIFF(MONTH,?mulaiDenda,DATE_ADD(?pdtgl_ttp_2,INTERVAL 1 DAY)) AS jbulan','cbulandenda')
*!*		nBuland =val(cbulandenda.jbulan)
*!*		nlDendapkb = nBuland 
*!*		jbulan = VAL(cbulandenda.jbulan)

*!*	    *MESSAGEBOX(TRANSFORM(jbulan))
...
*!*	    *MESSAGEBOX('awal:'+TRANSFORM(hariAwal)+' ahir:'+TRANSFORM(hariAhir))
*!*		
...
*!*				dltgl_kuitansi = Iif(dltgl_dft < dltgl_kuitansi, dltgl_dft , dltgl_kuitansi)
			...
			dltgl_kuitansi = Iif(!Isnull(dltgl_notice), dltgl_notice, dltgl_dft)
			dltgl_kuitansi = Iif(dltgl_dft < dltgl_notice, dltgl_dft , dltgl_notice)
...
*!*				messagebox(transform(nlDendapkb_2))
			*messagebox(transform(nlDendapkb_2))
...
*disini
				...
				Case rbh_btk    && rubah bentuk
					lnPajak = lnNilai_tanpa_bobot * Iif(laArray(i,2)='11', 0.09, 0.01)
				Case lcPlat = 'KUNING' AND clGuna='1'
					lnPajak = lnNilai_tanpa_bobot * Iif(laArray(i,2)='11', Iif(clJenisKend= 'F' Or clJenisKend ='G', 0.5, 0.3) * 0.09, Iif(clJenisKend= 'F' Or clJenisKend ='G', 0.5, 0.3) * 0.01 ) &&
			...
			IF clJenis = '702'
				lnPajak = (( lnNilai_tanpa_bobot - 2000000 ) * Iif(laArray(i,2)='11', 0.09, 0.01 )) + Iif(laArray(i,2)='11',(2000000* 0.10),0)
			ENDIF
		...
		Case  laArray(i,2) = '21' .Or. laArray(i,2) = '22' 		&& Hitung Denda BBNKB I & II
			...
			If llDendaBbn
				...
				Case lcPlat = 'KUNING' && Umum
					lnPajak  = lnNilai_tanpa_bobot * Iif( laArray(i,2) = '21', 0.13, 0.01 ) * Iif(clJenisKend= 'F' Or clJenisKend ='G', 0.5,0.3) * (Iif(nlDendabbn>2,nlDendabbn * 0.02,0))
				Case lcPlat = 'MERAH' && merah
					lnPajak  = lnNilai_tanpa_bobot * Iif( laArray(i,2) = '21', 0.13, 0.01 ) * (Iif(nlDendabbn>2,nlDendabbn * 0.02,0))
				Otherwise && plat merah & hitam
				...
				If lcPlat = 'MERAH'
					...
					If !perpanjangan
						...
						If (llDendaPkb And !propk) Or (llDendaPkb And propk And clBulanPkb > 12) Or (llDendaPkb And lcDumb)
							...
							*lnPajak     = (lnPkb * nTahun *  0.25)   + (lnPkb * nTahun * nlDendapkb * 0.02) && 2%
							*lnPajak     = (lnPkb * 1 *  0.25)   + (lnPkb * 1 * jbulan * 0.02) && 2%
							
							lnPajak     = lnPkb * (Iif((jbulan*0.02)+0.25>0.73,0.73,(jbulan*0.02)+0.25))
							...
							laArray(i,4) = Iif((Year(dltgl_notice_sd) > Year(dltgl_notice)) ,pembulatan(lnPajak),0)
							denda_biasa  = Iif((Year(dltgl_notice_sd) > Year(dltgl_notice)) ,pembulatan(lnPajak),0)
					...
					Else
						...
						If (llDendaPkb And !propk) Or (llDendaPkb And propk And clBulanPkb > 12) Or (llDendaPkb And lcDumb)
							...
							lnPajak     = lnPkb * nTahun * (Iif((jbulan*0.02)+0.25>0.73,0.73,(jbulan*0.02)+0.25))
							*lnPajak      = (lnPkb * nTahun *  0.25) + (lnPkb * nTahun *  jbulan * 0.02) && 2%
...
*mungkin disini
					...
					If !perpanjangan
						...
						If (llDendaPkb And !propk) Or (llDendaPkb And propk And clBulanPkb > 12) Or (llDendaPkb And lcDumb)
							...
							laArray(i,4) = Iif((Year(dltgl_notice_sd) > Year(dltgl_notice)) ,pembulatan(lnPajak),0)
*!*								messagebox(transform(lnPkb))
*!*								messagebox(transform(nTahun))
*!*								messagebox('!perpanjangan')
						Endif
...
*!*								messagebox('perpanjangan')
		...
		Case laArray(i,2) = '24' && denda swd
			...
			**************	
			...
			nlDendapkb_2    = Iif(nBulan_2 < 1, 0,nBulan_2)
*!*				messagebox(transform(nlDendapkb_2))
*!*				messagebox(transform(nBulan32_2))
*!*				messagebox(transform(nHari32_2))
*!*				messagebox(transform(nYear32_2))
*!*				messagebox(transform(nBulan32_2))
